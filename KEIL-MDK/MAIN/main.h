...
#define DEBUG_TEST
#ifdef DEBUG_TEST
//#define AK_LED_1         NRF_GPIO_PIN_MAP(0,13)
//#define AK_LED_2         	NRF_GPIO_PIN_MAP(0,14)
//#define AK_LED_3          NRF_GPIO_PIN_MAP(0,15)
//#define AK_LED_4         	NRF_GPIO_PIN_MAP(0,16)
#include "nrf_delay.h"
#include "nrf_gpio.h"
typedef struct
{
	uint32_t RunCnt;
	uint32_t BleAdvEvtCnt;
	uint32_t KeepaliveCnt;
	uint32_t LowPowerCnt;
	uint32_t IPCCRxCnt;
	
	uint32_t LoraLpCnt;
	uint32_t LoraTimeSliceCnt;
	uint32_t LoraTxTimeoutCnt;
	uint32_t LoraIdleCnt;
	uint32_t LoraDelayTime;
	
	uint32_t TestFlag;
	uint32_t ProcedureRunCycle;
	uint32_t ProcedureRunCnt;
	
	uint32_t BLE_ACT_Cnt;
	uint32_t RF_ACT_Cnt;
	uint32_t R802_ACT_Cnt;
	uint32_t BELWU_ACT_Cnt;
	uint32_t BHWF_ACT_Cnt;
	
	uint32_t RandomNum[30];
	
//	EXTI_HandleTypeDef hexti;
}Debug_ParamTypedef;
extern Debug_ParamTypedef Debug_Param;
#endif
/* USER CODE END EFP */
